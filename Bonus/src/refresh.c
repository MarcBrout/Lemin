...
** Last update Thu Apr 21 17:48:10 2016 THEIS Paul
*/
...
void	anim_ant(int id_room_start, int id_room_end, t_info *info)
{
  ...
  c_x = (info->element[id_room_end].x - info->element[id_room_start].x) / 10;
  c_y = (info->element[id_room_end].y - info->element[id_room_start].y) / 10;
  x = 0;
  y = 0;
  while (i  <= 10)
    {
      update_screen(info);
      put_ant_screen(info->element[id_room_start].x + (i * c_x),
		     info->element[id_room_start].y + (i * c_y), info);
      usleep(info->speed);
      i++;
  }
  SDL_Flip(info->screen);
...
void	del_ant_in_room(char *id_del, t_info *info, char *room)
{
  ...
  info->element[id_room].nbr_ants -= 1;
